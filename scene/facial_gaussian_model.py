...
class FacialGaussianModel(GaussianModel):
    def __init__(self, sh_degree, shape_len=300, exp_len=100):
        ...
        self._flame_lmks = torch.empty(0)
    ...
    def init_flame_mesh(self, init_flame_params, all_flame_params, cull_global_neighborhood=True):
        mesh_vertices, mesh_lmks = self.flame_forward(init_flame_params, 0)
        mesh_vertices = mesh_vertices[0]
        mesh_lmks = mesh_lmks[0]
        ...
        dist2 = torch.clamp_min(distCUDA2(mesh_vertices).float().cuda(), 0.0000001)
        scales = torch.log(torch.sqrt(dist2))[..., None].repeat(1, 3)
        ...
        self._flame_lmks = mesh_lmks # These change frame-to-frame and are not a param
    ...
    def replace_tensor_to_optimizer(self, tensor, name):
        flame_tensor = tensor[:len(self._flame_xyz)]
        global_tensor = tensor[len(self._flame_xyz):]
        if name in self.split_param_groups:
            flame_param = super().replace_tensor_to_optimizer(flame_tensor, f"flame_{name}")
            global_param = super().replace_tensor_to_optimizer(global_tensor, f"global_{name}")

            if name == "xyz":
                (None, global_param[name])
            else:
                return (flame_param[f"flame_{name}"], global_param[f"global_{name}"])
        else:
    ...
    def reset_opacity(self):
        ...
        self._global_opacity = optimizable_tensors[1]

    def _prune_optimizer(self, mask):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            if "flame" in group["name"]: # Do not remove flame data
                continue
            elif "global" in group["name"]:
    ...
    def prune_points(self, mask):
        ...
        valid_points_mask[:len(self._flame_xyz)] = True # Never prune any FLAME mesh points
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            if "flame" in group["name"]: # Do not augment flame data
                continue
    ...
    def update_flame_xyz(self, flame_params, t):
        flame_vertices, flame_lmks = self.flame_forward(flame_params, t)
        self._flame_xyz = flame_vertices[0]
        self._flame_lmks = flame_lmks[0]
